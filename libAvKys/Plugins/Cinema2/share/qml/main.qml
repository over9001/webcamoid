...
GridLayout {
    ...
    Slider {
        id: sldStripSize
        value: cinema2.stripSize
        stepSize: 0.01
        to: 1
        Layout.fillWidth: true

        onValueChanged: cinema2.stripSize = value
    }
    SpinBox {
        id: spbStripSize
        value: multiplier * cinema2.stripSize
        ...
        onValueModified: cinema2.stripSize = value / multiplier
    }

    // Configure strip color.
    Label {
        text: qsTr("Color Hi Ryan")
    }
    RowLayout {
        Layout.columnSpan: 2
        ...
        AK.ColorButton {
            currentColor: fromRgba(cinema2.stripColor)
            title: qsTr("Choose the strips color")
            showAlphaChannel: true

            onCurrentColorChanged: cinema2.stripColor = toRgba(currentColor)
        }
    }
}
